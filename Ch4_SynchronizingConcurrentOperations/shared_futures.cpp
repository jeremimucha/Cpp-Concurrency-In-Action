...
// --- Using std::shared_future for signalling ready-state to multiple threads
    ...
    auto fun1 = [&t1_ready_promise, &start, ready_future]() //-> std::chrono::duration<double, std::nano>
    {
        t1_ready_promise.set_value();   // make the promise ready
        ready_future.wait();    // waits for signal from main()
        return std::chrono::high_resolution_clock::now() - start;
    };

    auto fun2 = [&t2_ready_promise, &start, ready_future]() //-> std::chrono::duration<double, std::nano>
    {
